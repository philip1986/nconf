...
//
// ### function dir (key, options)
// #### @key {string|Object} Fully qualified options, name of dir store, or path.
// #### @options {string|Object} **Optional** Full qualified options, or path.
// Adds a new `File` store to this instance. Accepts the following options
//
//    nconf.file({ dir: 'path/to/config/dir', env: 'dev' });
//    nconf.file('path/to/config/dir');
//    nconf.file('userconfig', 'path/to/config/dir');
//    nconf.file('userconfig', { dir: 'path/to/config/dir', env: 'dev' });
//
Provider.prototype.dir = function (key, options) {
  if (arguments.length == 1) {
    options = typeof key === 'string' ? { dir: key } : key;
    key = 'dir';
  }
  else {
    options = typeof options === 'string'
